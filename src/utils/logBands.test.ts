...
describe('logBands utilities', () => {
  ...
  describe('freqToBin and binToFreq', () => {
    ...
    it('should convert frequency to bin correctly', () => {
      ...
      expect(freqToBin(sampleRate / 2, sampleRate, fftSize)).toBe(fftSize / 2 - 1)
      expect(freqToBin(1000, sampleRate, fftSize)).toBe(Math.floor(1000 * fftSize / sampleRate))
    })
    ...
    it('should convert bin to frequency correctly', () => {
      ...
      expect(binToFreq(1000, sampleRate, fftSize)).toBeCloseTo(1000 * sampleRate / fftSize)
    })
  ...
  describe('aggregateBands', () => {
    it('should aggregate frequency data correctly', () => {
      ...
      const result = aggregateBands(freqData, sampleRate, fftSize, bands, 'mean')
    ...
    it('should handle different reducer types', () => {
      ...
      const meanResult = aggregateBands(freqData, sampleRate, fftSize, bands, 'mean')
      const maxResult = aggregateBands(freqData, sampleRate, fftSize, bands, 'max')
  ...
  describe('snapToGrid', () => {
    it('should snap to octave fraction grid', () => {
      ...
      expect(Math.abs(result1010 - 1000) < Math.abs(result1010 - 1050)).toBe(true)
    })
