...
export function useAudioGraph() {
  ...
  async function init() {
    try {
      ...
      // @ts-expect-error - Support for webkitAudioContext on older Safari
      ...
      channelSplitter = new ChannelSplitterNode(audioCtx.value, {
        numberOfOutputs: 2,
      })
      channelMerger = new ChannelMergerNode(audioCtx.value, {
        numberOfInputs: 2,
      })
  ...
  function updateAudioRouting(activeChannels: ActiveChannels) {
    ...
    console.log(
      `Audio routing updated: ${effectiveMode} mode, L:${activeChannels.left} R:${activeChannels.right}`
    )
  }
  ...
  function setBoost(enabled: boolean) {
    ...
    console.log(
      `Boost ${enabled ? "enabled" : "disabled"}: ${targetGain}x gain (${enabled ? 20 : 0}dB)`
    )
  }
